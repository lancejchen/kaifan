...
<div class="cl">
<!--{if !$_G['forum']['allowspecialonly']}--><a href="forum.php?mod=post&action=newthread&fid=$_G[fid]" {if ($_GET[sortid] || !$special) && $_GET['action'] != 'edit' && $_GET['action'] != 'reply' && !$sortid}class="current"{/if}>{lang send_threads}</a><!--{/if}-->
<!--{if $_G['group']['allowpostpoll']}--><a href="forum.php?mod=post&action=newthread&fid=$_G[fid]&special=1" {if !$_GET[sortid] && $special == 1}class="current"{/if}>{lang post_newthreadpoll}</a><!--{/if}-->
