...
0
|| checktplrefresh('./template/webshow_mtb0115/touch/forum/post.htm', './template/webshow_mtb0115/touch/common/seccheck.htm', 1405933886, '2', './data/template/2_2_touch_forum_post.tpl.php', './template/webshow_mtb0115', 'touch/forum/post')
...
$(this).attr('src', 'misc.php?mod=seccode&update=<?php echo $ran;?>&idhash='+ tmprandom +'&mobile=2');
        ...
        <button id="postsubmit" style="width:100px !important; background-color:#f2f9fd;" data-icon="check" data-inline="true" class="btn_pn <?php if($_GET['action'] == 'edit') { ?>" <!--<?php } else { ?>" <?php } ?>><span><?php if($_GET['action'] == 'newthread') { ?>发     表<?php } elseif($_GET['action'] == 'reply') { ?>回复<?php } elseif($_GET['action'] == 'edit') { ?>保存<?php } ?></span></button>
        </div>
