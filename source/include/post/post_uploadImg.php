 ...
 * for upload image
 * Requirements:
 *  TODO check the reference (by post maybe, 漏洞： 可能恶意传图。
 * TODO use the right template
 *
 */
...
if(!submitcheck('topicsubmit', 1, 0, 0)) {
    ...
    $imgpid = $_GET['imgpid'];
    //show template
    getgpc('infloat') ? include template('forum/post_infloat') : include template('forum/imgUpload');
...
*/
    $bfmethods[] = array('class' => 'extend_thread_image', 'method' => 'beforeUploadImg');
    $afmethods[] = array('class' => 'extend_thread_image', 'method' => 'uploadImgWeixin');
    ...
    //$modthread->attach_before_methods('newImage', $bfmethods);
    $modthread->attach_after_methods('newImage', $afmethods);

    $return = $modthread->newImage($params);
    $tid = $_GET['imgtid'];
    $pid = $_GET['imgpid'];
